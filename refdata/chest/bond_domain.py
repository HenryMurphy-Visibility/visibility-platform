...
def close_bond_lots(investment: str, location: str, quantity: float, local: float, book: float, closing_method: str,
    ...
    investment_lots = [(k[:5] + v) for k, v in list(bs_entries.items()) if
                       k[1] == investment and k[5] == location and (v[0] < 0 and k[4] == ls or v[0] > 0 and k[4] == ls) and  k[6] == "Cost"]
    ...
    for location in locations:
        lots_in_location = [(k[0], k[1], k[2], k[3], v[0], v[1], v[2]) for k, v in bs_entries.items() if
                            k[1] == investment and (v[0] < 0 and ls == "s" or v[0] > 0 and ls == "l") and k[6] == "Cost"]
...
def bond_lot_iterator(investment, sub_ledger):
...
def bond_lot_iterator_by_location(investment, sub_ledger):
...
def buy_bond(portfolio, investment, location, quantity, local, book, journal_entries, sub_ledger, tranid, transaction,
    ...
    je = Journals(portfolio, investment, tranid, tradedate, ls, location, financial_account, quantity, local, book, None, None, tranid,
    ...
    ls = "s"
    financial_account = "AccruedInterestPayable" # use tranid for payable receivable to close
    je = Journals( portfolio, payment_currency, tranid, tradedate, ls, location, financial_account, -accrued_local, -accrued_local, -accrued_book, None, None, tranid,
                      transaction, tradedate, settledate, kdbegin, kdend, ibor_date, "Asset/Liability")
    sub_ledger.post_journal_entry(je)

    ls = "l"
    financial_account = "PurchasedInterest"  # use tranid for payable receivable to close
    je = Journals(portfolio, investment, tranid, tradedate, ls, location, financial_account, accrued_local, accrued_local, accrued_book, None, None, tranid,
                  transaction, tradedate, settledate, kdbegin, kdend, ibor_date, "Asset/Liability")
    sub_ledger.post_journal_entry(je)
...
def sell_bond(portfolio, investment, location, quantity, local, book, closing_method, journal_entries,
    ...
    ls = "l"
    financial_account = "AccruedInterestRecevable"  # use tranid for payable receivable to close
    je = Journals(portfolio, payment_currency, tranid, tradedate, ls, location, financial_account, accrued_local, accrued_local,
    ...
    je = Journals(portfolio, investment, tranid, tradedate, ls, location, financial_account, -accrued_local, -accrued_local,
                  -accrued_book, None, None, tranid,
                  transaction, tradedate, settledate, kdbegin, kdend, ibor_date, "Asset/Liability")
    sub_ledger.post_journal_entry(je)
    ...
    for lot in lots_returned:
        ...
        sub_ledger.post_journal_entry(closed_lot_journal)
        ...
        if pgain_local != 0:
            ...
            sub_ledger.post_journal_entry(pg_journal)
...
def short_bond(portfolio, investment, location, quantity, local, book, journal_entries, sub_ledger, tranid,
    ...
    ls = "l"
    financial_account = "AccruedInterestReceivable"  # use tranid for payable receivable to close
    je = Journals(portfolio, payment_currency, tranid, tradedate, ls, location, financial_account, accrued_local, accrued_local,
                  accrued_book, None, None, tranid,
                  transaction, tradedate, settledate, kdbegin, kdend, ibor_date, "Asset/Liability")
    sub_ledger.post_journal_entry(je)
...
def bond_coupon(portfolio, investment, journal_entries, sub_ledger, tranid,
    ...
    for location, positions in net_positions.items():
        for position_type, qty in positions.items():
            ...
            bcoup = Journals(portfolio, payment_currency, tranid, tradedate, ls, location, faal, 0, coupon, coupon,
                             None, None,tranid,
                             transaction, tradedate, settledate, kdbegin, kdend, ibor_date, "Asset/Liability")
