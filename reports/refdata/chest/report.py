...
# def update_running_balances(journal_entries, fields_to_update):
#     # Update running balances logic
#     pass
...
def tax_lot_appraisal(bookkeeping_list, ledger_space, stat_repo, edate, fname):
    ...
    for entry in bookkeeping_list:
        ...
        # Fetch market values using the new method in the Journals class
        mktval_local, mktval_book = Journals.fetch_market_values(ledger_space.journal_entries, edate,
                                                                 portfolio, investment, lotid, "MarketVal")



        if mktval_local is None or mktval_book is None:
            # Handle the case when market values are not found
            print(f"No market values found for {portfolio}, {investment}, {lotid} on {edate}")
            continue

        # Calculate the edate_fx_rate
        edate_fx_rate = mktval_book / mktval_local if mktval_local != 0 else 0

        # Calculate the new columns
        price_gl_local = mktval_local - local
        price_gl_book = price_gl_local * edate_fx_rate
        fx_gl_book = mktval_book - book - price_gl_book

        # Construct the row with cost, market value, and the new calculated information
        booksp_row = [
            portfolio, investment, lotid, tax_date, ls, location, financial_account,
            quantity, local, book, mktval_local, mktval_book,
            price_gl_local, price_gl_book, fx_gl_book  # Add the new columns here
        ]

        bookkeeping_space_list.append(booksp_row)

    # Convert the bookkeeping space list into a DataFrame
    df = pd.DataFrame(bookkeeping_space_list, columns=[
        'Portfolio', 'Investment', 'Lot_ID', 'Tax_Date', 'Ls', 'Location',
        'Financial_Account', 'Quantity', 'Local_Cost', 'Book_Cost', 'MktVal_Local', 'MktVal_Book', 'PriceGLLocal',
        'PriceGLBook', 'FXGLBook'
    ])

    # Define a function to get the 'IsCurrency' attribute
    def get_is_currency(investment):
        return ledger_space.get_investment_attribute('AIF', investment, 'IsCurrency')

    # Apply the function to the 'Investment' column to create the 'IsCurrency' column
    df['IsCurrency'] = df['Investment'].apply(get_is_currency)

    # Sort by 'IsCurrency' first (descending) and then by 'Investment' (ascending)
    df = df.sort_values(by=['IsCurrency', 'Investment'], ascending=[False, True])

    # Calculate subtotals by 'IsCurrency' and 'Investment'
    investment_totals = df.groupby(['IsCurrency', 'Investment']).agg({
        'Quantity': 'sum',
        'Local_Cost': 'sum',
        'Book_Cost': 'sum',
        'MktVal_Local': 'sum',
    ...
    # Add constant columns to subtotals
    investment_totals['Portfolio'] = 'Subtotal'
    investment_totals['Lot_ID'] = ''
    investment_totals['Tax_Date'] = ''
    investment_totals['Ls'] = ''
    investment_totals['Location'] = ''
    investment_totals['Financial_Account'] = ''

    # Append the subtotals to the original DataFrame
    df_with_subtotals = pd.concat([df, investment_totals], ignore_index=True)

    # Sort the resulting DataFrame to place subtotals correctly
    df_with_subtotals = df_with_subtotals.sort_values(by=['IsCurrency', 'Investment', 'Portfolio'],
                                                      ascending=[False, True, False]).reset_index(drop=True)

    print(df_with_subtotals)

    # Calculate grand totals
    grand_totals = investment_totals.agg({
        'Book_Cost': 'sum',
        'MktVal_Book': 'sum',
        'PriceGLBook': 'sum',
        'FXGLBook': 'sum',
    ...
    grand_totals['Lot_ID'] = ''
    grand_totals['Tax_Date'] = ''
    grand_totals['Ls'] = ''
    grand_totals['Location'] = ''
    grand_totals['Financial_Account'] = ''
    ...
    output_filepath = f"{outputpath}/{fname}"
    df_with_totals.to_excel(output_filepath, index=False, sheet_name='Sheet1')

    # Apply your existing formatting function
    apply_standard_formatting(output_filepath, "Sheet1")
...
# Example usage
# journals_by_tranid(journal_entries, bookkeeping_space, sdate, edate, fname, "YourSpecificInvestmentName")
...
def cost_basis_balance_sheet(bookkeeping_space, edate, fname):
    bookkeeping_space_list = []
    for entry in bookkeeping_space:
        # Unpack all nine elements directly from the list
        ...
        # Append the constructed row to the bookkeeping_space_list
        bookkeeping_space_list.append(booksp_row)


    df = pd.DataFrame(bookkeeping_space_list, columns=[
        'Portfolio', 'Investment', 'LS', 'Financial _Account',
         'Quantity', 'Local', 'Book'
    ])
    ...
    df_merged = fetch_and_map_groupings(df, "C:/Users/hjmne/PycharmProjects/chest/refdata/investment_master.csv",
                                        "C:/Users/hjmne/PycharmProjects/chest/refdata/chart_of_accounts.csv")
   # df_merged.drop('Investment', axis=1, inplace=True)

    df_primary_structure= df_merged[['Portfolio', 'Investment','BSGroup', 'BSGroupName', 'asset_class', 'LS', 'Financial Account', 'Quantity', 'Local', 'Book']]
    sorted_df = df_primary_structure.sort_values(by=['BSGroup', 'asset_class', 'Financial Account'], ascending=[True, True, True])

    # Step 4 - important calc grand totals first. What you are grouping by will dictate subtotal_columns vs total_cols
    group_by_columns = ['BSGroup']
    total_columns = ['Book']
    subtotal_columns = ['Book']

    #Always do grand_totals before sub-totaling
    grand_totals = calculate_grand_totals(sorted_df, total_columns)
    bookkeeping_records = df.set_index([
        'Portfolio', 'Investment', 'Lot_ID', 'Tax_Date', 'LS', 'Location', 'Financial_Account']).T.to_dict('list')
    ...
    #
    # Step 3 - merge with COA/Inv Master Columns
    # Example usage of fetch_and_map_groupings within your code
    df_valued = value_positions(bookkeeping_records, sub_ledger, price_data, fx_data, edate, mark_type)

    # Step 3 - merge with COA/Inv Master Columns
    df_valued = df_valued.drop(columns=['Tax_Date', 'Location'])
    df_subtotals = subtotal_data(sorted_df, subtotal_columns, group_by_columns)

    df_subtotals.sort_values(by=['BSGroup', 'asset_class', 'Financial Account', 'LS'], inplace=True)
    df_subtotals = insert_subtotals(df_subtotals, ['BSGroup', 'asset_class',  'Financial Account','LS'], subtotal_columns)

    # Append the Grand Totals row to the DataFrame with subtotals
    df_final_report = pd.concat([df_subtotals, grand_totals], ignore_index=True)

    # Step 5: Save the DataFrame to an Excel file
    fnamechoice = f"C:/Users/hjmne/PycharmProjects/chest/reports/CostBasisBalanceSheet"+fname+".xlsx"
    df_final_report.to_excel(fnamechoice, index=False)


    report_title = "Cost Basis Balance Sheet" +fname # Replace with your report title
    # Assuming 'Portfolio' is the column name where portfolio names are stored.
    # This filters the DataFrame to exclude any rows where 'Portfolio' is 'Subtotal' or 'Grand Totals'
    # and then gets the unique values in the 'Portfolio' column.
    unique_portfolios = df_final_report[
...
def position_report_by_sector(bookkeeping_space, sub_ledger, edate, kd, fname):
    # Step 1: Create the DataFrame from the list of entries
    bookkeeping_space_list = []
    for entry in bookkeeping_space:
        portfolio, investment, lotid, tax_date, ls, location, financial_account, quantity, local, book, notional, oface = entry
...
def position_report_by_asset_class(bookkeeping_space, edate, fname):
    ...
    bookkeeping_space_list = []
    for entry in bookkeeping_space:
        portfolio, investment, tax_lot_num, ls, location, financial_account, quantity, local, book = entrym
        booksp_row = [portfolio, investment, tax_lot_num, ls, location, financial_account, quantity, local, book]
        bookkeeping_space_list.append(booksp_row)
...
def dump_fund_structures_journals(journal_entries, filename):
    ...
    account_to_bucket = dict(zip(coa_df['SystemName'], coa_df['BSGroup']))

    # Map the Financial Account in df to its corresponding BSGroup
    df['BSGroup'] = df['Financial Account'].map(account_to_bucket)

    # Map the Financial Account to its Economic Bucket and add as a new column

    # Create sorted entries and subtotals
    #df.sort_values(by=['BSGroup', 'Entity', 'Financial Account', 'IBOR Date'], inplace=True)
...
def create_raw_accounting_reports(journal_entries, usefordiff, fname, period_start, period_cutoff, sub_ledger, account_to_bucket):
    ...
    # Ensure that account_to_bucket is a dictionary mapping 'SystemName' to 'BSGroup'.
    account_to_bucket = dict(zip(coa_df['SystemName'], coa_df['BSGroup']))

    # Map external data to DataFrame
    df = map_external_data(df, 'Financial Account', account_to_bucket, 'BSGroup')

    # Compute the 'BSGroup' column
    df['BSGroup'] = df['Financial Account'].map(account_to_bucket)

    # Sort the DataFrame by 'BSGroup'
    df = df.sort_values(by='BSGroup')

    # Group by 'BSGroup' and sum the 'Book' column for the original DataFrame
    bsgroup_subtotals = df.groupby("BSGroup")["Book"].sum().reset_index()

    # Rename the columns in the subtotal DataFrame
    bsgroup_subtotals.columns = ["BSGroup", "Book Subtotal"]

    # Merge the subtotals back to the original DataFrame
    df = pd.merge(df, bsgroup_subtotals, on="BSGroup", how="left")
