...
def close_futures_lots(investment: str, location: str, quantity: float, local: float, book: float,
    ...
    for lot in investment_lots:
        ...
        if lot_quantity == closed_qty:
            ...
            # Close a partial lot
            local = closed_qty * lot_notional / lot_quantity
        ...
        closed_lots.append((portfolio, investment, lotid, tax_date, ls,  closed_qty, local, book, closed_proceeds))
        lots_left -= 1
...
def buy_future(portfolio, investment, location, quantity, local, book, journal_entries, sub_ledger, tranid, transaction,
    ...
    ibor_date = tradedate
    tax_date = tradedate
    je = Journals(portfolio, investment, tranid, tax_date, ls, location, financial_account, quantity, local, book, notional, None, tranid,
    ...
    if (local !=0):
        ...
        je = Journals( portfolio, payment_currency, tranid, tradedate, ls, location, financial_account, -local, -local, -book,None, None, tranid,
                          transaction, tradedate, settledate, kdbegin, kdend, ibor_date, "Asset/Liability")
        sub_ledger.post_journal_entry(je)
...
def sell_future(portfolio, investment, location, quantity, local, book, closing_method, journal_entries,
                ...
                tdate_fx, proceeds_notional, price
                ):
    # investment_type = subspace.get_information_field("AIF", "investment_type") if subspace else None
    # pricing_factor = subspace.get_information_field("AIF", "pricing_factor") if subspace else 1.0
    ...
    lots_returned = close_futures_lots(investment, location, quantity, local, book, proceeds_notional, closing_method,
                                       tradedate, journal_entries, sub_ledger, "l", tranid)

    currency_local = 0
    currency_book = 0

    for lotinfo in lots_returned:
        portfolio, investment, lotid, tax_date,  ls, closed_qty, closed_local, closed_book, closed_proceeds = lotinfo
        ...
        invclose = Journals(portfolio, investment, lotid,  tax_date, "l", location, "Cost", -closed_qty,
                            0, 0,-closed_local, None, tranid, transaction, tradedate, settledate, kdbegin, kdend, tradedate,
                            "Asset/Liability")
        ...
        if pgain_local != 0:
            pgclose = Journals(portfolio, investment, lotid, tax_date, "l", location, "PriceGainInvestment", 0,
                               -pgain_local, -pgain_book, None, None, tranid, transaction, tradedate, settledate, kdbegin, kdend, tradedate,
        ...
        if glbook != 0:
            glclose = Journals(portfolio, investment, lotid, tax_date, "l", location, "FXGainInvestment",  0, 0,
                               -glbook, None, None,  tranid, transaction, tradedate, settledate, kdbegin, kdend, tradedate,
                               "Revenue/Expense/Capital")
    ...
    je = Journals(portfolio, payment_currency, tranid, tradedate, ls, location, financial_account,
                  currency_local, currency_local, currency_book, None, None, tranid,
                  transaction, tradedate, settledate, kdbegin, kdend, tradedate, "Asset/Liability")
    sub_ledger.post_journal_entry(je)



def short_futures(portfolio, investment, location, quantity, local, book, notional, journal_entries, sub_ledger, tranid, transaction,
               tradedate, settledate, kdbegin, kdend, payment_currency, put_call, strike, underlying, tdate_fx):
    # Create a new je Open IBM
    ...
    ibor_date = tradedate
    tax_date = tradedate
    je = Journals(portfolio, investment, tax_date, ls, location, financial_account, -quantity, -local, -book, -notional, None, tranid,
                      transaction, tradedate, settledate, kdbegin, kdend, ibor_date, "Asset/Liability")
    sub_ledger.post_journal_entry(je)
    ...
    return
def cover_futures(portfolio, investment, location, quantity, local, book, closing_method, journal_entries, sub_ledger, tranid,
                transaction, tradedate, settledate, kdbegin, kdend, payment_currency, put_call, strike, underlying, tdate_fx):
    ls = "s"
    ...
    lots_returned = close_futures_lots(investment, location, -quantity, -local, -book, closing_method, tax_date,
                                      journal_entries, sub_ledger, 's', tranid)

    # Loop over the closed lots and post the corresponding journal entries
    for lotinfo in lots_returned:
        portfolio, investment, tax_date, closed_qty, closed_local, closed_book, closed_proceeds = lotinfo
        ...
        if pgain_local != 0:
            pgclose = Journals(portfolio, investment, tax_date, ls, location, "PriceGainInvestment", 0, -pgain_local,
                         -pgain_book, tranid, transaction, tradedate, settledate, kdbegin, kdend, ibor_date, "Revenue/Expense/Capital")
            sub_ledger.post_journal_entry(pgclose)

        if glbook != 0:
            glclose = Journals(portfolio, investment, tax_date, ls, location, "FXGainInvestment", 0, 0, -glbook,
