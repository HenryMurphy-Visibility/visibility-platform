...
#    compute_daily_twr(journal_entries, 'portfolio', portfolio_name)
    create_performance_sheets(journal_entries, portfolio_name)
# Given functions...
...
-                                        finalized_inputs['Currency_Flows_Book']
    ...
    # Fetch data and calculate returns for S&P 500
    sp500 = yf.download('^GSPC', start='2023-01-01', end='2023-12-31')
    sp500['S&P 500'] = sp500['Adj Close'].pct_change() * 100
    sp500.reset_index(inplace=True)
    indices['S&P 500'] = sp500[['Date', 'S&P 500']].set_index('Date')

    # Fetch data and calculate returns for NASDAQ
    nasdaq = yf.download('^IXIC', start='2023-01-01', end='2023-12-31')
    nasdaq['NASDAQ'] = nasdaq['Adj Close'].pct_change() * 100
    nasdaq.reset_index(inplace=True)
    indices['NASDAQ'] = nasdaq[['Date', 'NASDAQ']].set_index('Date')

    # Fetch data and calculate returns for Russell 2000
    russell2000 = yf.download('^RUT', start='2023-01-01', end='2023-12-31')
    russell2000['Russell 2000'] = russell2000['Adj Close'].pct_change() * 100
    russell2000.reset_index(inplace=True)
    indices['Russell 2000'] = russell2000[['Date', 'Russell 2000']].set_index('Date')

    # Fetch data and calculate returns for IEF 30-Year Treasury
    ief = yf.download('IEF', start='2023-01-01', end='2023-12-31')
    ief['IEF30YrTreas'] = ief['Adj Close'].pct_change() * 100
    ief.reset_index(inplace=True)
    indices['IEF30YrTreas'] = ief[['Date', 'IEF30YrTreas']].set_index('Date')
    ...
    nasdaq = yf.download('^IXIC', start='2022-01-01', end='2023-12-31')
    ...
    russell2000 = yf.download('^RUT', start='2022-01-01', end='2023-12-31')
    # Calculate daily returns in percentage for each index
    russell2000['Russell 2000'] = russell2000['Adj Close'].pct_change() * 100
    russell2000.reset_index(inplace=True)
    # Merge each index's data into your DataFrame
    ...
    ief = yf.download('IEF', start='2022-01-01', end='2023-12-31')
    # Calculate daily returns in percentage for each index
    ief['IEF30YrTreas'] = ief['Adj Close'].pct_change() * 100
    ief.reset_index(inplace=True)
    # Merge each index's data into your DataFrame
    ...
    return finalized_inputs, summary_finalized_inputs, indices
...
def create_summary(twr_result, indices, level):
    ...
    summary = twr_result.groupby(level).agg(
        ...
        Currency_Flows_Book=('Currency_Flows_Book', 'sum'),
        Close_CF_Book=('Close_CF_Book', 'sum'),
        Open_CF_Book=('Open_CF_Book', 'sum'),
        Book_To_Date_Percent=('BookToDate_Percent', 'last'),
        SP500=('S&P 500', lambda x: (np.prod(1 + x) - 1) * 100),  # Convert to %
        NASDAQ=('NASDAQ', lambda x: (np.prod(1 + x) - 1) * 100),  # Convert to %
        Russell2000=('Russell 2000', lambda x: (np.prod(1 + x) - 1) * 100),  # Convert to %
        IEF30YrTreas=('IEF30YrTreas', lambda x: (np.prod(1 + x) - 1) * 100),  # Convert to %
    ).reset_index()

    # Calculate `Category_Flows_Book` by summing the component flows
    summary['Category_Flows_Book'] = (
        summary['Currency_Flows_Book'] +
        summary['Close_CF_Book'] +
        summary['Open_CF_Book']
    )

    # Drop the individual component columns
    summary.drop(columns=['Currency_Flows_Book', 'Close_CF_Book', 'Open_CF_Book'], inplace=True)

    # Analytics integration (Beta and R²)
    analytics_by_level = calculate_analytics(twr_result, indices, level)
    analytics_df = pd.DataFrame.from_dict(analytics_by_level, orient='index').stack().reset_index()
    analytics_df.columns = [level, 'Index', 'Metrics']
    analytics_df = pd.concat([analytics_df.drop(['Metrics'], axis=1), analytics_df['Metrics'].apply(pd.Series)], axis=1)

    # Merge analytics into the summary
    summary = pd.merge(summary, analytics_df, on=level, how='left')

    # Scale Beta to percentage form
    summary['Beta (%)'] = summary['Beta'] * 100

    # Add Risk-Adjusted Return (RAR)
    summary['Risk_Adjusted_Return'] = summary.apply(
        lambda row: row['Book_To_Date_Percent'] - (row['Beta'] * row['SP500']) if row['Beta'] != 0 else 0,
        axis=1
    )

    # Refactor Risk-Adjusted Return (divide by 100)
    summary['Risk_Adjusted_Return (%)'] = summary['Risk_Adjusted_Return']

    # Add Alpha column (Risk-Adjusted Return minus Book_To_Date_Percent)
    summary['Alpha'] = summary['Risk_Adjusted_Return'] - summary['Book_To_Date_Percent']

    # Drop intermediate RAR column if not needed
    summary.drop(columns=['Risk_Adjusted_Return'], inplace=True)

    columns_order = (
            summary.columns[:4].tolist() +  # First 4 columns
            ['Category_Flows_Book'] +  # Fifth column
            [col for col in summary.columns if
             col not in summary.columns[:4].tolist() + ['Category_Flows_Book', 'Alpha']] +  # Remaining columns
            ['Alpha']  # Ensure Alpha is last
    )
    summary = summary[columns_order]

    return summary

def calculate_analytics(twr_result, indices, level):
    analytics_by_level = {}

    grouped_returns = twr_result.groupby(level)

    for level_value, group in grouped_returns:
        print(f"Processing analytics for {level}: {level_value}")
        portfolio_returns = group.groupby('ibor_date')['TWR_Book'].sum()

        min_date = portfolio_returns.index.min()
        max_date = portfolio_returns.index.max()
        ...
        for index_name, index_data in indices.items():
            ...
            if aligned_data.empty:
                print(f"No data for {index_name} at {level}={level_value}. Skipping.")
                continue

            portfolio_aligned = aligned_data.iloc[:, 0]
            index_aligned = aligned_data.iloc[:, 1]
            ...
            try:
                ...
                print(f"Error in calculation for {index_name}: {e}")
                beta, r_squared = np.nan, np.nan

            level_analytics[index_name] = {
                'Beta': beta,
                'R²': r_squared
            }

        analytics_by_level[level_value] = level_analytics

    return analytics_by_level
...
def create_performance_sheets(journal_entries, portfolio_name):
    levels = ['investment', 'portfolio', 'Asset_Class', 'Analyst', 'Sector', 'Industry']

    output_dir = "BASE_PATH/reports/"
    os.makedirs(output_dir, exist_ok=True)

    for level in levels:
        detail_fname = os.path.join(output_dir, f"{portfolio_name}_{level}ReturnsDet.xlsx")
        summary_fname = os.path.join(output_dir, f"{portfolio_name}_{level}ReturnsSum.xlsx")

        # Compute TWR (your existing logic)
        twr_result, _, indices = compute_daily_twr(journal_entries, level, level, False)
        ...
        # Create the summary
        summary_df = create_summary(twr_result, indices, level)

        # Filter for S&P 500 after analytics are calculated
        if 'Index' in summary_df.columns:
            summary_df = summary_df[summary_df['Index'] == 'S&P 500']
        else:
            print("Warning: 'Index' column not found. Skipping filtering.")

        # Write detail and summary to Excel without formatting
        try:
            twr_result.to_excel(detail_fname, sheet_name='Detail', index=False)
            summary_df.to_excel(summary_fname, sheet_name='Summary', index=False)
            print(f"Files saved: {detail_fname} and {summary_fname}")
        except Exception as e:
            print(f"Error saving files: {e}")

        # If the level is 'portfolio', create a zip file of the Excel files
        if level == 'portfolio':
            perf_dir = os.path.join(output_dir, "perf")
            os.makedirs(perf_dir, exist_ok=True)

            zip_filename = os.path.join(perf_dir, f"{portfolio_name}_portfolioReturns.zip")
            with zipfile.ZipFile(zip_filename, 'w') as zipf:
                zipf.write(detail_fname, arcname=os.path.basename(detail_fname))
                zipf.write(summary_fname, arcname=os.path.basename(summary_fname))
...
#                 print("Warning: 'Index' column not found. Skipping filtering.")
#
#             # Write detail and summary to Excel
...
# ---- Aggregate Performance Reports by Level ----
import pandas as pd
import os
from zipfile import BadZipFile

def aggregate_performance_reports_by_level(report_dir, output_dir):
    """
    Aggregates performance reports from a specified directory by categories
    and generates a separate summary report for each category level.
    Args:
        report_dir (str): The directory containing performance reports.
        output_dir (str): The directory to save aggregated summary reports.
    """
    ...
    # Scan the directory for .xlsx and .csv files
    for file in os.listdir(report_dir):
        ...
        try:
            # Determine the file type and read the report into a DataFrame
            if file.endswith(".xlsx"):
                workbook = pd.ExcelFile(file_path, engine='openpyxl')
                available_sheets = workbook.sheet_names
                sheet_to_use = "Summary" if "Summary" in available_sheets else available_sheets[0]
            ...
            # Extract portfolio name from the file name
            portfolio_name = os.path.basename(file).split('_')[0]
            df['Portfolio'] = portfolio_name  # Add portfolio name to each row
            all_summaries.append(df)

        except BadZipFile:
            print(f"BadZipFile error: {file_path} is not a valid Excel file. Skipping.")
            continue
        except Exception as e:
    ...
    if all_summaries:
        ...
        # Define report categories for aggregation
        report_categories = ["Asset_Class", "Analyst", "Sector", "Industry", "investment"]

        # Iterate over each category and generate a summary report
        for level in report_categories:
            if level not in combined_df.columns:
                print(f"Warning: {level} not found in combined DataFrame. Skipping.")
                continue

            # Aggregate numeric fields by the category level
            numeric_columns = combined_df.select_dtypes(include='number').columns
            level_summary = combined_df.groupby([level, 'Portfolio'])[numeric_columns].mean().reset_index()

            # Save the summary report for this level
            output_file = os.path.join(output_dir, f'{level}_Summary.xlsx')
            level_summary.to_excel(output_file, index=False, engine='openpyxl')

        print("Aggregation complete. Reports saved in the specified directory.")
    else:
        print("No matching data found to aggregate.")
...
if __name__ == "__main__":
    ...
    report_files = glob(os.path.join(report_dir, "*ReturnsSum.xlsx"))
    aggregate_performance_reports_by_level(report_files, output_dir)